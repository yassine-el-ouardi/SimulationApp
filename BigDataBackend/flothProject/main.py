...
#         input_df2 = combined_df_concentrate.reindex(columns=scalerX.feature_names_in_)
#         scaled_input2 = scalerX.transform(input_df2)
#         raw_prediction2 = model.predict(scaled_input2)
#         final_prediction2 = scalerY.inverse_transform(raw_prediction2)

#         concentrate_predictions2 = final_prediction2[:, :10].tolist()
#         tailing_predictions2 = final_prediction2[:, 10:20].tolist()
#         combined_predictions_concentrate2 = [static_variables[i][:14] + concentrate_predictions2[i][:10] for i in range(len(static_variables))]
#         combined_predictions_tailing2 = [static_variables[i][:14] + tailing_predictions2[i][:10] for i in range(len(static_variables))]
...
def predict():
    try:
       ...
       # input_df = input_df[selected_features]
        ...
        # Use the output of the first model for the second model
        combined_df_concentrate = pd.DataFrame(combined_predictions_concentrate, columns=scalerX.feature_names_in_)
        combined_df_tailing = pd.DataFrame(combined_predictions_tailing, columns=scalerX.feature_names_in_)
